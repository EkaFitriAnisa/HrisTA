...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="px-5 py-3.5 border-b border-gray-100 bg-gray-50/50">
                <form method="GET" action="{{ route('hrd.clearance.index') }}" class="flex flex-wrap gap-2 items-center">
                    ...
                    @if (request('search') || request('status'))
                        <a href="{{ route('hrd.clearance.index') }}"
                        ...
                        @forelse ($clearances as $cl)
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors align-top">
                                ...
                                <td class="px-5 py-4 text-right">
                                    @if ($currentRole === 'HRD' && $allAssetsReturned && $cl->status !== 'approved')
                                        <form method="POST" action="{{ route('hrd.clearance.finalize', $cl->id) }}" class="inline">
                                            @csrf
                                            <button type="button" onclick="confirmFinalize({{ $cl->id }}, '{{ $cl->karyawan?->nama }}')"
                                                class="h-8 px-3 rounded-lg border border-emerald-300 bg-emerald-600 hover:bg-emerald-700
                                                text-white text-xs font-semibold flex items-center gap-1.5 transition-colors">
                                                <i class="bi bi-patch-check-fill"></i>
                                                Finalisasi
                                            </button>
                                        </form>
                                    @elseif ($myPendingAssets->count() > 0)
    ...
    <form id="form-finalize" method="POST" class="hidden">@csrf</form>
@endsection
    ...
    <script>
        ...
        function confirmApprove(id, name) {
            Swal.fire({
                ...
                form.action = `{{ url('hrd/clearance/aset') }}/${id}/approve`;
                form.submit();
            });
        }
        ...
        function confirmReject(id, name) {
            Swal.fire({
                ...
                form.action = `{{ url('hrd/clearance/aset') }}/${id}/reject`;
                form.submit();
            });
        }
        ...
        function confirmFinalize(id, name) {
            Swal.fire({
                title: 'Finalisasi Clearance?',
                html: `Clearance milik <strong>${name}</strong> akan ditandai sebagai <em>selesai / approved</em>.<br><span class="text-xs text-gray-400">Tindakan ini tidak dapat dibatalkan.</span>`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: 'rgb(5, 150, 105)',
                cancelButtonColor: '#6b7280',
                confirmButtonText: '<i class="bi bi-patch-check-fill me-1"></i> Ya, Finalisasi',
                cancelButtonText: 'Batal',
                reverseButtons: true,
            }).then(r => {
                if (!r.isConfirmed) return;
                const form = document.getElementById('form-finalize');
                form.action = `{{ url('hrd/clearance') }}/${id}/finalize`;
                form.submit();
            });
        }
    </script>
